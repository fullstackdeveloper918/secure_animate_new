jQuery(function ($) {
  ...
  window.ScrollEffects = function () {
    ...
    // Call functions
    // VerificÄm dacÄ body nu are clasele 'load-project-thumbs' Či 'load-page'
    if (
    ...
    if (isMobile()) {
      ...
      function setViewPortHeight() {
        ...
        gsap.set(
          '.smooth-scroll main, .has-parallax, .clapat-nav-wrapper, .clapat-slider-wrapper:not(.content-slider), .showcase-lists .clapat-sync-slider, .slider-fixed-content',
          { height: winHeight }
        );
      }
    ...
    gsap.utils.toArray('.clipped-image-wrapper').forEach((clippedImageWrapper) => {
      ...
      function setClippedImageWrapperProperties() {
        ...
        gsap.set(clippedImage, {
          height: window.innerHeight - document.querySelector('.clapat-header').offsetHeight * 2,
        });
        gsap.set(clippedImageContent, {
          paddingTop: window.innerHeight / 2 + clippedImageContent.offsetHeight,
        });
              ...
              gsap.fromTo(
                ...
                },
                {
                  ...
                  scrollTrigger: {
                    ...
                    start: function () {
                      const startPin =
                        window.innerHeight - (window.innerHeight - slide.offsetHeight) / 2;
                      return 'top +=' + startPin;
                    },
                    //end: "top 10%",
                    end: function () {
    ...
    gsap.set('.header-gradient', {
      ...
      onUpdate: function () {
        var bgColor = document.getElementById('clapat-page-content').getAttribute('data-bgcolor');
        document.querySelector('.header-gradient').style.maskImage =
          `linear-gradient(${bgColor}, transparent)`;
        document.querySelector('.header-gradient').style.webkitMaskImage =
          `linear-gradient(${bgColor}, transparent)`;
      },
    });
    ...
    $('#hero #hero-caption.landing-page-title .inner').mouseenter(function (e) {
      ...
      // AdÄugÄm prima imagine imediat la prima intrare
      $('#ball').append(
        '<img class="eyes-icon" src="images/eyes.png" style="transform:scale(0.75);">'
      );

      // FuncČie pentru schimbarea imaginilor ĂŽntre cele trei
      let imageIndex = 0;
      const images = [
      ...
      function switchImage() {
        const ball = $('#ball');
        ball.find('img').remove(); // EliminÄm imaginea curentÄ

        // AdÄugÄm urmÄtoarea imagine din array
        ball.append(images[imageIndex]);

        // IncrementÄm indexul Či resetÄm la ĂŽnceputul array-ului dacÄ e cazul
        imageIndex = (imageIndex + 1) % images.length;
      }

      // SetÄm intervalul pentru schimbarea imaginii la fiecare 3 secunde
      const imageInterval = setInterval(switchImage, 1000);

      // Oprim schimbarea icoanelor cĂ˘nd mouse-ul pÄrÄseČte zona
    ...
    if (!$('body').hasClass('disable-ajaxload')) {
      if ($('#itemsWrapper').hasClass('webgl-fitthumbs')) {
        if ($('#itemsWrapper').length > 0) {
          ...
          const thumbs = [
            ...itemsWrapper.querySelectorAll('img.grid__item-img:not(.grid__item-img--large)'),
          ];
  ...
  window.Shortcodes = function () {
    ...
    $('#copy-email').on('click', function () {
      ...
      // VerificÄm dacÄ navigator.clipboard este disponibil
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard
          ...
          .then(function () {
            $('#ball p').remove();
            $('#ball').append(
              '<p class="center-first" style="color:#000">' + clickedMessage + '</p>'
            );
          })
          .catch(function (err) {
            $('#ball p').remove();
        ...
        // Fallback: creÄm un textarea pentru a copia emailul
        var $temp = $('<textarea>');
        $('body').append($temp);
    ...
    $('#copy-email')
      .on('mouseenter', function () {
        ...
        if (!$(this).hasClass('copied')) {
          ...
          $('#ball p').remove();
          $('#ball').append(
            '<p class="center-first" style="color:#000">' + $this.data('hover-message') + '</p>'
          );
          ...
          $('#ball p').remove();
          $('#ball').append(
            '<p class="center-first" style="color:#000">' + $this.data('clicked-message') + '</p>'
          );
        }
      })
      .on('mouseleave', function () {
            ...
            if (maxduration > 0) {
              for (var i = 0; i < video_object[0].buffered.length; i++) {
                if (
                  ...
                  var perc =
                    (video_object[0].buffered.end(video_object[0].buffered.length - 1 - i) /
                      maxduration) *
                      100 +
                    '%';
          ...
          setTimeout(function () {
            ...
            if (!isMobile() && !$('body').hasClass('disable-cursor')) {
              ...
              $('.parallax-wrap').mouseenter(function (e) {
                var currentElement = $(this);
                var primaryColor =
                  currentElement.data('border-color') || $('body').data('primary-color');
                var scaleValue, borderWidthValue;
                if (currentElement.hasClass('1x')) {
                  scaleValue = 0.6;
                  borderWidthValue = '4px';
              ...
              function parallaxIt(e, parent, target, movement) {
                ...
                gsap.to(target, {
                  ...
                  x: ((relX - boundingRect.width / 2) / boundingRect.width) * movement,
                  y:
                    ((relY - boundingRect.height / 2 - scrollTop) / boundingRect.height) * movement,
                  ease: Power2.easeOut,
                });
              }
              ...
              function parallaxCursor(e, parent, movement) {
                ...
                pos.x = rect.left + rect.width / 2 + (relX - rect.width / 2) / movement;
                pos.y =
                  rect.top + rect.height / 2 + (relY - rect.height / 2 - scrollTop) / movement;
                gsap.to(ball, { duration: 0.3, x: pos.x, y: pos.y });
              }
