          ...
          onUpdate: function (self) {
            ...
            spans.forEach(function (span, spanIndex) {
              ...
              var opacity = gsap.utils.clamp(
                0,
                1,
                gsap.utils.mapRange(spanStart, spanEnd, 0, 1, progress)
              );
  ...
  function PageLoadActions() {
    ...
    if (!isMobile()) {
      $('#page-nav .next-ajax-link-page').on('mouseenter', function () {
        ...
        $('#ball').addClass('with-blur');
        $('#ball').append(
          '<p class="center-first" style="color:#000">' + $this.data('centerline') + '</p>'
        );
      });
    ...
    if (!$('body').hasClass('disable-ajaxload')) {
      if ($('#project-nav').hasClass('auto-trigger')) {
        if (
          !(typeof window.ReachBottomArr === 'undefined' || window.ReachBottomArr === null) &&
        ...
        gsap.to($('#project-nav'), {
          scrollTrigger: {
            trigger: $('#project-nav'), // Elementul care declanČeazÄ efectul
            start: () => startValue,
            toggleClass: 'active-link', // Clasa pe care o adaugÄ Či o Čterge
          },
        });
      } //End Auto Trigger
    ...
    if ($('#project-nav').length > 0) {
      ...
      ScrollTrigger.create({
        ...
        onLeaveBack: function () {
          ...
          gsap.set(
            '#clapat-logo img, .classic-menu .flexnav li a, .button-wrap.menu, .button-icon-link',
            { clearProps: 'all' }
          );
        },
        scrub: true,
...
---------------------------------------------------*/
  ...
  function LazyLoad() {
    ...
    setTimeout(function () {
      $('.clapat-header').removeClass('white-header');
      $('body').removeClass(
        'load-project-thumb load-project-thumb-with-title load-project-thumb-from-slider load-next-page grid-open'
      );
      setTimeout(function () {
        imagesLoaded('body', function () {
  ...
  function ShowcasePortfolio() {
    gsap.utils.toArray('.clapat-item .pixels-cover').forEach((pixelWrapper, index) => {
      ...
      gsap.to(pixelAnimation, {
        ...
        opacity: 0,
        delay: function () {
          // GenerÄm un delay random ĂŽntre 0 Či 0.5 secunde
    ...
    if ($('.showcase-portfolio').length > 0) {
            ...
            if (colorClass !== '') {
              projects.forEach(function (project) {
                if (!project.classList.contains(colorClass)) {
                  ...
                  project.classList.remove('not-filtered');
                  hasFilteredItems = true; // Elemente filtrate gÄsite
                }
              });
            } else {
              projects.forEach(function (project) {
      ...
      if ($('#filters-wrapper').length > 0) {
        $('#itemsWrapper').on('click', '.toggle-filters, #close-filters', function () {
          ...
          var filtersHeight =
            4 + filtersLi.toArray().reduce((acc, li) => acc + $(li).outerHeight(true), 0);
      ...
      if ($('.filp-grid').length > 0) {
        if (!isMobile()) {
          ...
          const moveThumbsParent = document.querySelectorAll(
            '.flip-thumbs-wrapper .flip-move-thumb'
          );
          ...
          const overlappingThumbs = document.querySelectorAll(
            '.showcase-portfolio .clapat-item .img-mask'
          );

          let completedAnimations = 0; // VariabilÄ pentru a numÄra animaČiile completate
          const moveAnimations = []; // Array pentru a stoca animaČiile
          ...
          function animateElements(moveThumbs, overlappingThumbs, moveThumbsParent) {
            moveThumbs.forEach((moveThumb, index) => {
              ...
              const moveAnimation = Flip.from(state, {
                ...
                absolute: true,
                onComplete: function () {
                  completedAnimations++; // IncrementÄm numÄrul de animaČii completate

                  // OpriČi animaČia de flip doar dacÄ toate animaČiile sunt complete
                  if (completedAnimations === moveThumbs.length) {
                    setTimeout(function () {
                      moveAnimations.forEach((anim) => anim.kill()); // OpriČi toate animaČiile
                    }, 200);
              ...
              moveAnimations.push(moveAnimation); // SalveazÄ animaČia pentru a o putea opri mai tĂ˘rziu
  ...
  function ShowcaseCarousel() {
    if ($('.showcase-carousel').length > 0) {
      ...
      if (!isMobile()) {
        ...
        $('.showcase-carousel .clapat-slide .slide-inner-height')
          .on('mouseenter', function () {
    ...
    if ($('.showcase-gallery').length > 0) {
      ...
      if (!$('body').hasClass('show-loader')) {
        ...
        setTimeout(function () {
          ...
          gallerySlideClasses.forEach(function (gallerySlideClass, index) {
            var gallerySlide = $(
              '.showcase-gallery .clapat-slider ' + gallerySlideClass + ' .slide-inner-height'
            );
      ...
      const hideGrid = () => {
        ...
        const currentImageInner = document.querySelector(
          '.slider-zoom-wrapper .trigger-item .section-image'
        );
      ...
      $('.clapat-slide .slide-inner-height').on('mouseenter', function () {
        ...
        if (liElementOnLeave && firstEnter) {
          onLeave(liElementOnLeave, function () {
            var onEnterShuffle = gsap.utils.toArray(
              '.external-caption .slide-title[data-caption="' + slide + '"]'
            );
    ...
    if ($('.projects-list-wrapper').length > 0) {
      ...
      if (!$('body').hasClass('disable-ajaxload')) {
        $('.projects-list-captions li span').on('click', function () {
          ...
          // AnimaČia pentru 'li'-ul click-uit
          animateShuffleForLi(clickedIndex);

          // AnimaČia pentru 'li'-urile din stĂ˘nga Či dreapta
          for (var i = 1; i < onLeaveShuffle.length; i++) {
            if (clickedIndex - i >= 0) {
              animateShuffleForLi(clickedIndex - i); // Elemente spre stĂ˘nga
            }
